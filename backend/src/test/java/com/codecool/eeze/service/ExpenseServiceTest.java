...
class ExpenseServiceTest {
    ...
    void givenValidExpenseID_whenUpdateExpense_thenReturnUpdatedExpenseWithIdAmountDateCategoryDTO() {
        // GIVEN
        UUID expenseId = UUID.randomUUID();
        ExpenseWithIdAmountDateCategoryDTO dto = new ExpenseWithIdAmountDateCategoryDTO(expenseId, 600.00, LocalDate.of(2025, 3, 8), "Transport");

        Expense mockedExpense = mock(Expense.class);
        TransactionCategory updatedCategory = mock(TransactionCategory.class);

        given(expenseRepository.findExpenseByPublicId(expenseId)).willReturn(Optional.of(mockedExpense));
        given(transactionCategoryRepository.getTransactionCategoryByName("Transport")).willReturn(updatedCategory);
        given(expenseRepository.save(any(Expense.class))).willReturn(mockedExpense);

        given(mockedExpense.getPublicId()).willReturn(expenseId);
        given(mockedExpense.getAmount()).willReturn(600.00);
        given(mockedExpense.getTransactionDate()).willReturn(LocalDate.of(2025, 3, 8));
        given(mockedExpense.getTransactionCategory()).willReturn(updatedCategory);
        given(updatedCategory.getName()).willReturn("Transport");

        // WHEN
        ExpenseWithIdAmountDateCategoryDTO result = expenseService.updateExpense(expenseId, dto);

        // THEN
        assertThat(result).isNotNull();
        assertThat(result.amount()).isEqualTo(600.00);
        assertThat(result.transactionDate()).isEqualTo(LocalDate.of(2025, 3, 8));
        assertThat(result.expenseCategory()).isEqualTo("Transport");
